    ...
    try {
        ...
        for (const reminder of reminders) {
            try {
                ...
                if (!reminder.user) {
                    console.log(`⚠️ Skipping reminder ${reminder._id} - no user associated`);
                    continue;
                }
                ...
                const user = await User.findById(userId).select("deviceToken firstName lastName email");

                if (!user) {
                    console.log(`⚠️ Skipping reminder ${reminder._id} - user ${userId} not found`);
                    continue;
                }

                // Check if user has a device token
                if (!user.deviceToken || user.deviceToken.trim() === "") {
                    console.log(`⚠️ Skipping reminder ${reminder._id} - user ${userId} has no device token`);
                    continue;
                }
                ...
                try {
                    ...
                    console.log(`✅ Push notification sent for reminder ${reminder._id} to user ${user._id}`);
                } catch (error: any) {
                    const errCode = error?.errorInfo?.code;

                    if (errCode === "messaging/registration-token-not-registered") {
                        console.log(`🚫 Invalid device token for user ${user._id} - clearing token`);
                        ...
                        console.error(`❌ Error sending push notification for reminder ${reminder._id}:`, errCode || error);
                    }
                }
            } catch (error) {
        ...
        console.log(`✅ Reminder notification job completed. Processed ${reminders.length} reminder(s).`);
    } catch (err) {
